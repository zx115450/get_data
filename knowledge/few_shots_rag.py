...
def compress_few_shot_content(content: str) -> str:
    ...
    validator 的 read*+skipBlanks+readEof/ensuref（不验空白格式）；不贴大段代码，
    ...
    if "ensuref" in text:
        ...
        val_parts.append("仅范围题：read* + skipBlanks + readEof（inf.strict=false，禁止裸 readEof）")
    if "readEof" in text or "readEoln" in text or "registerValidation" in text:
        val_parts.append("宽松 read*+skipBlanks+readEof（字段顺序以本题标程为准；不验空格/换行）")
    bullets.append("validator：" + "；".join(val_parts))

    bullets.append(
        "禁令：只借 registerGen/opt/skipBlanks+readEof/各轴小中大全组合等通用骨架；"
        "禁止借范例输入字段形状与范例约束常数；"
