...
class App:
    ...
    def _describe_done_job_local(self, job_id: str) -> dict | None:
        """本地判断 job 是否已成功可下载。"""
        try:
            from server.runners.resume import describe_done_job
        ...
        *,
        ...
        if prefer:
            ...
            if info:
                return [info]
        try:
            from server.runners.resume import find_matching_done_jobs, text_hash
...
def main():
    ...
    app = App(root)
    session = problem_store.load_session()
    if session:
        app.apply_workspace(session, confirm=False)
