...
用法（在项目根目录）：
    python setup.py
    # 或：python scripts/bootstrap_env.py

Windows 也可双击 setup.bat。

完成后用 PyCharm 打开本项目，解释器选 .venv，运行 gui.py 或 python -m gui 即可。

说明：本文件是交互式环境初始化脚本，不是 setuptools 打包入口；
包元数据见 pyproject.toml（pip install -e ".[dev]"）。
"""
...
# 若从 scripts/bootstrap_env.py 运行，ROOT 为仓库根
...
# 国内镜像，减轻 PyPI 超时；editable 用 --no-build-isolation 避免再拉构建隔离环境
PIP_INDEX_URL = "https://pypi.tuna.tsinghua.edu.cn/simple"


def _pip(vpy: Path, *args: str) -> None:
    """在 venv 里执行 pip，默认走清华镜像。"""
    cmd = [str(vpy), "-m", "pip", *args]
    if "-i" not in args and "--index-url" not in args:
...
def ensure_venv_and_deps() -> None:
    ...
    _print(f"升级 pip（镜像: {PIP_INDEX_URL}）…")
    _pip(vpy, "install", "--upgrade", "pip", "setuptools", "wheel")
    ...
    _pip(vpy, "install", "-r", str(REQ_PATH))

    _print('editable 安装本项目 (pip install -e ".[dev]" --no-build-isolation) …')
    try:
        _pip(vpy, "install", "-e", ".[dev]", "--no-build-isolation")
    except subprocess.CalledProcessError as e:
        _print(
            f"editable 安装失败 (exit={e.returncode})，已跳过。"
            "日常仍可用 python gui.py；稍后可手动："
        )
        _print(
            f'  pip install -e ".[dev]" -i {PIP_INDEX_URL} --no-build-isolation'
        )
    _print("依赖安装完成。")
...
def main() -> None:
    ...
    if _yes_no("是否现在创建虚拟环境并安装依赖？", default=True):
        try:
            ...
            _print(
                f"安装失败 (exit={e.returncode})，请检查网络或手动:\n"
                f"  pip install -r requirements.txt -i {PIP_INDEX_URL}\n"
                f'  pip install -e ".[dev]" -i {PIP_INDEX_URL} --no-build-isolation'
            )
