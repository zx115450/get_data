...
输出格式：一个整数。
数据范围：n in [1,1e5]，ai in [-1e9,1e9]，本范例 15 组（实际 count 由覆盖需求自定，不得小于 15），覆盖 n=1、n=max、全相等、降序；其余组按 index 在 [1,1e5] 上均匀分层（既有小也有大）。
...
输出格式：一个整数。
数据范围：n in [1,100]，W in [1,1000]，wi,vi in [1,1000]；本范例 count=15（实际由覆盖需求自定，≥15）。
...
输出格式：一个整数。
数据范围：n,m in [1,200]，aij in [-1e9,1e9]；本范例 count=15（实际由覆盖需求自定，≥15）。
...
输出格式：q 行，每行一个整数。
数据范围：n,q in [1,1e5]，ai in [-1e9,1e9]；本范例 count=15（实际由覆盖需求自定，≥15）。
...
输出格式：一个整数。
数据范围：n,q in [1,1e5]；本范例 count=15（实际由覆盖需求自定，≥15）。
