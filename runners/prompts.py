...
def build_gen_fixer_task(
    ...
    parts = [
        ...
        "2. 根据失败类型判断根因：gen TIMEOUT/MEMORY → 先查 uni≤域基数，"
        ...
        "TIMEOUT：① 先核 FAIL index 解轴（规模须 (index/3)%3）；"
        "② 再按读标程方向调 FAIL type/同类最大档（多数压 K≤200 有限域复用；"
        "标程随单种体量变差则反向调；保留小中档多样；禁止略微收窄；满规模≠满状态）。\n"
...
def build_coder_rewrite_task(
    ...
    parts = [
        ...
        "- gen TIMEOUT / MEMORY（O(n^2) 枚举、预建大池子、或 uni>域基数的去重 while）；\n"
        ...
        "遵守第 4/7 节分层与解轴硬约定（规模=(index/3)%3）；"
        "若因 TIMEOUT 重写：先核解轴，再按读标程方向调相关最大档（多数压 K≤200）。\n"
...
def build_batch_fixer_task(
    ...
    parts.append(
        ...
        "   - std FAILED / TIMEOUT / MEMORY：修 gen——"
        "TIMEOUT 时先核解轴（规模=(index/3)%3），再按读标程方向调最大档"
        "（多数压 K≤200；少数反向调），保留小中档多样。\n"
