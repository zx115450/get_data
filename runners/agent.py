...
def _build_planner_few_shot_block(
    ...
    return (
        ...
        "generator.h 的 gen()/edges()、validator 的 read*+skipBlanks+readEof/ensuref（inf.strict=false）。\n"
    ...
    if plan_path.is_file():
        ...
        if not _plan_looks_complete(plan_text):
            ...
            retry_prompt = (
                ...
                "6. validator（ensuref 或 read*+skipBlanks+readEof；不验空白格式） "
        ...
        if plan_text.strip() and not _plan_looks_complete(plan_text):
            ...
            retry2 = (
                ...
                "- 保留 8 节与 skipBlanks+readEof / include（禁止裸 readEof；不要求 readEoln）。\n"
        ...
        if _plan_too_long(plan_text):
            ...
            compress_prompt = (
                ...
                "尤其保留第 4 节状态密度分层、第 5 节打满上界的 ≤K复用、第 6 节 ensuref/skipBlanks+readEof（不验格式）、"
